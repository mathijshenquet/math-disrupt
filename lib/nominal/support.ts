...
export class FiniteSet implements NominalSet {
  ...
  private map(
    mapper: (value: Name, key: Name, iter: Set<Name>) => Name
  ): FiniteSet {
...
export class CofiniteSet implements NominalSet {
  ...
  isSubset(other: Support): boolean {
    ...
    else return this.excluded.isSuperset(other.excluded);
  }

  union(other: Support): Support {
    if (other instanceof FiniteSet)
      return new CofiniteSet(this.excluded.subtract(other.included));
    else return new CofiniteSet(this.excluded.intersect(other.excluded));
  }
...
export const Support = {
  ...
  isSubset(sup: Support, sub: Support) {
    ...
    else if (sup instanceof CofiniteSet) return sup.isSubset(sub);
    else return false;
  },
...
export function support(term: Term): Support {
  if (term instanceof Name) return Support.singleton(term);
  else if (term instanceof Hole) return term.pmss;
  else if (term instanceof Form) return support(term.parts);
  ...
  } else throw new Error("Unreachable in #support");
}
