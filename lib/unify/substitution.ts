...
export function Substitution(X?: Hole, t?: Term): Substitution {
  if (X === undefined || t === undefined) return Map();
...
export function substitute(sub: Substitution, term: Term): Term {
  if (term instanceof Hole) return sub.get(term, term);
  else if (term instanceof Name) return term;
  else if (term instanceof Form)
    return new Form(
      term.head,
      <Composite>substitute(sub, term.parts),
      term.sort
    );
  else if (term instanceof Composite)
    return new Composite(
      term.elements.map(substitute.bind(null, sub)),
      term.sort
    );
  else if (term instanceof Bind)
    return new Bind(term.name, <Form>substitute(sub, term.term), term.sort);
  else throw new Error("Unreachable in #substitute");
}
