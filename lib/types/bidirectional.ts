...
export class TypeJudgement {
  ...
  substitute(sub: Substitution): this {
    let substitution: (x: Term) => Term = substitute.bind(null, sub);
...
export class Compat {
  ...
  solve(checker: TypeChecker, ctx: Context): Substitution {
    if (!is(this.sub, this.sub)) throw new Error("Types not compatible");
    return Map();
  }

  substitute(sub: Substitution): Compat {
    let substitution: (x: Term) => Term = substitute.bind(null, sub);
    return new Compat(substitution(this.sub), substitution(this.sup));
  }
}
...
export class Lookup {
  ...
  substitute(sub: Substitution): Compat {
    return new Compat(substitute(sub, this.name), substitute(sub, this.type));
  }
}
...
export class TypeChecker {
  ...
  synth(e: Expr, ctx: Context = Map()): Type {
    let rule = this.synthRules.get(e.sort); // get the relevant rule
  ...
  check(T: Type, t: Term, ctx: Context = Map()): boolean {
    ...
    let rule = this.checkRules.get(T.sort); // get the relevant rule
