...
import { TypeChecker, Check, Synth, Compat, Lookup } from "./bidirectional";
import $ from "./signature";

const typechecker = new TypeChecker();
export default typechecker;

let x = $.unknown("x", "name"),
  n = $.unknown("n", "numeral");
...
// check a synth
typechecker.addCheck(T, e, [new Synth(e, S), new Compat(S, T)]);
...
// bound var
typechecker.addSynth($.op("var", x), S, [new Lookup(x, S)]);

// application
typechecker.addSynth(
  $.op("apply", f, s),
  $.op("sub", $.bind(x, T), $.op("annotate", s, S)),
  [new Synth(f, pi), new Check(S, s)]
);
...
// nat succ
typechecker.addSynth($.op("s", e), $.op("nat"), [new Check(e, $.op("nat"))]);
